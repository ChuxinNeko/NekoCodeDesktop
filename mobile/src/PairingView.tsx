...
export function PairingView({ initialEndpoint, busy, error, onPair, onError }: {
	...
	return <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-6 py-8">
		...
		<div className="flex w-full max-w-md flex-col gap-5 self-center">
			<div className="flex gap-1 rounded-lg bg-muted p-1" role="tablist" aria-label="配对方式">
				{(["scan", "manual"] as const).map((value) => <button key={value} role="tab" id={`pair-${value}-tab`} aria-selected={mode === value} aria-controls={`pair-${value}-panel`} disabled={disabled} onClick={() => { setMode(value); onError(null); }} className={`min-h-10 flex-1 rounded-md px-3 text-sm transition-colors ${mode === value ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"}`}>{value === "scan" ? "扫码配对" : "手动配置"}</button>)}
			</div>
			{mode === "scan" ? <section id="pair-scan-panel" role="tabpanel" aria-labelledby="pair-scan-tab" className="flex flex-col gap-5">
