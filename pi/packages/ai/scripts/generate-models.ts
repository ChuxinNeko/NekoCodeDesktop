...
async function loadModelsDevData(): Promise<Model<any>[]> {
	try {
		...
		// models.dev renamed the catalog to kimi-code-plan-cn / kimi-code-plan-global.
		const kimiCodePlan =
			data["kimi-code-plan-cn"] ?? data["kimi-code-plan-global"] ?? data["kimi-for-coding"];
		if (kimiCodePlan?.models) {
			const kimiModels = kimiCodePlan.models as Record<string, ModelsDevModel>;
