...
export interface BuildSystemPromptOptions {
	...
	/** Guideline bullets contributed by each tool, keyed by tool name. Included with default and custom prompts. */
	toolGuidelines?: Record<string, string[]>;
	/** Additional guideline bullets, included with default and custom prompts. */
...
/**
 * The active tools' usage contract on its own, for a custom prompt: none of the
 * default rules, which belong to the default persona.
 */
function buildToolGuidelines(
	...
	for (const rule of [...selectedTools.flatMap((name) => toolGuidelines[name] ?? []), ...promptGuidelines]) {
		const normalized = rule.trim();
...
export function buildSystemPromptSections(input: BuildSystemPromptOptions): SystemPromptSections {
	...
	if (customPrompt) {
		...
		// tools' usage contract.
		const guidelines = buildToolGuidelines(selectedTools, toolGuidelines, promptGuidelines);
		if (guidelines) promptSections.tool_guidelines = `Tool usage guidelines:\n${guidelines}`;
	} else {
